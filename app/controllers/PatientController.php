...
class PatientController extends Controller {
    ...
    private function gate(): void { $this->requireRole('patient'); }
    ...
    public function dashboard(): void {
        ...
        $rdvs = (new Consultation())->listerPourPatient($pid);
        $factures = (new Facture())->listerPourPatient($pid);
        $this->render('patient/dashboard', compact('rdvs','factures','pid'));
    }
    ...
    public function demander_consultation(): void {
        ...
        if ($_SERVER['REQUEST_METHOD'] === 'POST') {
            if (!$this->checkCsrf()) { $this->redirect('patient/demander_consultation'); }
            try {
    ...
    public function demander_sortie(): void {
        $this->gate();
        if ($_SERVER['REQUEST_METHOD'] === 'POST') {
            if (!$this->checkCsrf()) { $this->redirect('patient/demander_sortie'); }
            try {
                $_POST['patient_id'] = $this->currentPatientId();
    ...
    public function payer_facture(): void {
        ...
        $pid = $this->currentPatientId();
        if ($_SERVER['REQUEST_METHOD'] === 'POST') {
            if (!$this->checkCsrf()) { $this->redirect('patient/payer_facture'); }
