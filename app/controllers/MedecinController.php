...
class MedecinController extends Controller
{
    ...
    public function dashboard(): void
    {
        ...
        $medecin = $medecinId ? (new Personnel())->trouver($medecinId) : null;
        $nomComplet = $medecin ? trim('Dr. ' . $medecin['prenom'] . ' ' . $medecin['nom']) : ($_SESSION['user']['nom'] ?? 'Médecin');
        $specialite = $medecin['specialite'] ?? 'Médecin généraliste';

        // Statistiques
        $nbPatients = count($patients);
        $nbConsultations = count($consultations);
        $nbHospitalisations = (new Hospitalisation())->compter();
        $nbUrgences = (new Urgence())->compterEnAttente();

        // Dernières consultations
        $dernieresConsultations = (new Consultation())->listerRecentes(5);

        // Derniers examens
        $derniersExamens = (new Examen())->listerRecents(5);

        $this->render('medecin/dashboard', compact(
            'patients',
            'consultations',
            'nomComplet',
            'specialite',
