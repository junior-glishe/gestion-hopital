...
class AdministrateurController extends Controller {
    ...
    private function gate(): void { $this->requireRole('administrateur'); }
    ...
    public function dashboard(): void {
        ...
        $this->render('administrateur/dashboard', compact('patients','personnels','factures','total'));
    }

    public function enregistrer_patient(): void {
        $this->gate();
        if ($_SERVER['REQUEST_METHOD'] === 'POST') {
            if (!$this->checkCsrf()) { $this->redirect('administrateur/enregistrer_patient'); }
            try {
    ...
    public function gerer_personnel(): void {
        $this->gate();
        if ($_SERVER['REQUEST_METHOD'] === 'POST') {
            if (!$this->checkCsrf()) { $this->redirect('administrateur/gerer_personnel'); }
    ...
    public function planifier_rendezvous(): void {
        $this->gate();
        if ($_SERVER['REQUEST_METHOD'] === 'POST') {
            if (!$this->checkCsrf()) { $this->redirect('administrateur/planifier_rendezvous'); }
            try {
    ...
    public function affecter_chambre(): void {
        $this->gate();
        if ($_SERVER['REQUEST_METHOD'] === 'POST') {
            if (!$this->checkCsrf()) { $this->redirect('administrateur/affecter_chambre'); }
    ...
    public function generer_facture(): void {
        $this->gate();
        if ($_SERVER['REQUEST_METHOD'] === 'POST') {
            if (!$this->checkCsrf()) { $this->redirect('administrateur/generer_facture'); }
            try {
    ...
    public function enregistrer_paiement(): void {
        $this->gate();
        if ($_SERVER['REQUEST_METHOD'] === 'POST') {
            if (!$this->checkCsrf()) { $this->redirect('administrateur/enregistrer_paiement'); }
            try {
