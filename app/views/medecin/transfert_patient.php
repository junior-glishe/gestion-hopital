...
<?php $titre = "Transférer patient - MediTrace"; ob_start(); ?>
<style>
    body {
  ...
  <div class="flex">
    ...
    <aside class="w-72 bg-white shadow-lg min-h-screen sticky top-0">
      ...
      <nav class="p-4">
        ...
        <a href="index.php?p=medecin/transfert_patient" class="sidebar-item active flex items-center gap-3 px-3 py-2.5 rounded-lg text-gray-700 mb-1">
    ...
    <main class="flex-1">
      <div class="max-w-3xl mx-auto px-4 py-8">
        ...
        <div class="flex items-center gap-2 text-sm text-gray-500 mb-6">
          ...
          <span class="text-gray-800 font-medium">Transférer un patient ★</span>
        </div>
        ...
        <div class="bg-white rounded-2xl shadow-xl p-6 md:p-8">
          <div class="flex items-center gap-3 mb-6">
            ...
            <h1 class="text-xl md:text-2xl font-bold text-gray-800">Transférer un patient</h1>
            <span class="px-2 py-1 bg-purple-100 text-purple-700 text-xs rounded-full">Spécialiste</span>
          </div>

          <form action="" method="POST" class="space-y-5">
<?php /* CSRF */ ?><input type="hidden" name="_csrf" value="<?= htmlspecialchars($csrf_token ?? '', ENT_QUOTES) ?>">
            <div>
              <label class="block text-sm font-medium text-gray-700 mb-1">Patient à transférer *</label>
              <select name="patient" required class="w-full px-4 py-2.5 border border-gray-200 rounded-xl input-focus transition bg-white">
                ...
                <option value="patient1">Jean ADANDE - Cardiologie</option>
                <option value="patient2">Marie ZINSOU - Pédiatrie</option>
              </select>
            </div>

            <div>
              <label class="block text-sm font-medium text-gray-700 mb-1">Service destinataire *</label>
              <select name="service_destinataire" required class="w-full px-4 py-2.5 border border-gray-200 rounded-xl input-focus transition bg-white">
            ...
            <div>
              <label class="block text-sm font-medium text-gray-700 mb-1">Médecin destinataire *</label>
              <select name="medecin_destinataire" required class="w-full px-4 py-2.5 border border-gray-200 rounded-xl input-focus transition bg-white">
            ...
            <div>
              ...
              <textarea name="motif" rows="3" required class="w-full px-4 py-2.5 border border-gray-200 rounded-xl input-focus transition bg-white" placeholder="Raison médicale du transfert..."></textarea>
            </div>

            <div>
              <label class="block text-sm font-medium text-gray-700 mb-1">Résumé du dossier médical *</label>
              <textarea name="resume_dossier" rows="4" required class="w-full px-4 py-2.5 border border-gray-200 rounded-xl input-focus transition bg-white" placeholder="Antécédents, diagnostic, traitements en cours..."></textarea>
            </div>

            <div>
              <label class="block text-sm font-medium text-gray-700 mb-1">Date de transfert souhaitée *</label>
              <input type="date" name="date_transfert" required class="w-full px-4 py-2.5 border border-gray-200 rounded-xl input-focus transition bg-white">
            </div>

            <div>
              <label class="block text-sm font-medium text-gray-700 mb-1">Urgence du transfert</label>
              <div class="flex gap-4 mt-1">
                <label class="flex items-center gap-2"><input type="radio" name="urgence" value="normale" checked class="w-4 h-4 text-green-600"><span class="text-sm text-gray-700">Normale</span></label>
                <label class="flex items-center gap-2"><input type="radio" name="urgence" value="urgent" class="w-4 h-4 text-red-600"><span class="text-sm text-gray-700">Urgent (24h)</span></label>
              </div>
            </div>

            <div class="flex flex-col sm:flex-row gap-3 pt-4">
              <button type="submit" class="flex-1 py-3 bg-purple-600 text-white rounded-xl font-medium hover:bg-purple-700 transition">
        ...
        <!-- Demandes de transfert récentes -->
        <div class="bg-white rounded-2xl shadow-xl p-6 mt-6">
          <h3 class="font-semibold text-gray-800 mb-4">Demandes de transfert récentes</h3>
          <div class="space-y-3">
            <div class="flex justify-between items-center p-3 bg-gray-50 rounded-lg">
              <div>
                <p class="text-sm font-medium text-gray-800">Patient : ..... → Service : .....</p>
                <p class="text-xs text-gray-500">Demandé le ..... - Statut : <span class="text-yellow-600">En attente</span></p>
              </div>
              <a href="#" class="text-purple-600 text-xs hover:underline">Suivre</a>
            </div>
          </div>
        </div>
