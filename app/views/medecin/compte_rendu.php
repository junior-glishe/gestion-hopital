...
<?php $titre = "Compte-rendu hospitalisation - MediTrace"; ob_start(); ?>
<style>
    body {
  ...
  <div class="flex">
    ...
    <aside class="w-72 bg-white shadow-lg min-h-screen sticky top-0">
      ...
      <nav class="p-4">
        ...
        <a href="index.php?p=medecin/compte_rendu" class="sidebar-item active flex items-center gap-3 px-3 py-2.5 rounded-lg text-gray-700 mb-1">
    ...
    <main class="flex-1">
      <div class="max-w-4xl mx-auto px-4 py-8">
        ...
        <div class="flex items-center gap-2 text-sm text-gray-500 mb-6">
          ...
          <span class="text-gray-800 font-medium">Compte-rendu d'hospitalisation</span>
        </div>

        <div class="bg-white rounded-2xl shadow-xl p-6 md:p-8">
          <div class="flex items-center gap-3 mb-6">
            <div class="w-10 h-10 bg-indigo-100 rounded-xl flex items-center justify-center">
              <svg width="22" height="22" viewBox="0 0 24 24" fill="none" stroke="#4f46e5" stroke-width="1.8">
                <path d="M4 4H20V20H4V4Z"/>
                <path d="M8 7H16"/>
                <path d="M8 11H12"/>
              </svg>
            </div>
            <h1 class="text-xl md:text-2xl font-bold text-gray-800">Compte-rendu d'hospitalisation</h1>
          </div>
...
<?php /* CSRF */ ?><input type="hidden" name="_csrf" value="<?= htmlspecialchars($csrf_token ?? '', ENT_QUOTES) ?>">
            <div>
              ...
              <select name="patient" required class="w-full px-4 py-2.5 border border-gray-200 rounded-xl input-focus transition bg-white">
                <option value="">Sélectionnez un patient</option>
                <option value="patient1">Jean ADANDE - Hospitalisé du 10/04 au 17/04/2025</option>
                <option value="patient2">Marie ZINSOU - Hospitalisé du 12/04 au 15/04/2025</option>
              </select>
            </div>

            <div>
              <label class="block text-sm font-medium text-gray-700 mb-1">Date de rédaction *</label>
              <input type="date" name="date_redaction" required class="w-full px-4 py-2.5 border border-gray-200 rounded-xl input-focus transition bg-white">
            </div>

            <div>
              <label class="block text-sm font-medium text-gray-700 mb-1">Résumé de l'hospitalisation *</label>
              <textarea name="resume" rows="4" required class="w-full px-4 py-2.5 border border-gray-200 rounded-xl input-focus transition bg-white" placeholder="Motif, durée, évolution..."></textarea>
            </div>

            <div>
              <label class="block text-sm font-medium text-gray-700 mb-1">Diagnostic final *</label>
              <textarea name="diagnostic_final" rows="3" required class="w-full px-4 py-2.5 border border-gray-200 rounded-xl input-focus transition bg-white" placeholder="Diagnostic établi en fin d'hospitalisation..."></textarea>
            </div>

            <div>
              <label class="block text-sm font-medium text-gray-700 mb-1">Examens réalisés</label>
              <textarea name="examens_realises" rows="3" class="w-full px-4 py-2.5 border border-gray-200 rounded-xl input-focus transition bg-white" placeholder="Liste des examens effectués et résultats..."></textarea>
            </div>

            <div>
              <label class="block text-sm font-medium text-gray-700 mb-1">Traitements reçus</label>
              <textarea name="traitements_recus" rows="3" class="w-full px-4 py-2.5 border border-gray-200 rounded-xl input-focus transition bg-white" placeholder="Médicaments administrés..."></textarea>
            </div>

            <div>
              <label class="block text-sm font-medium text-gray-700 mb-1">Recommandations post-sortie *</label>
              <textarea name="recommandations" rows="4" required class="w-full px-4 py-2.5 border border-gray-200 rounded-xl input-focus transition bg-white" placeholder="Traitement, consultations de contrôle, activité physique, alimentation..."></textarea>
            </div>

            <div class="grid md:grid-cols-2 gap-4">
              <div>
                <label class="block text-sm font-medium text-gray-700 mb-1">Date de prochain contrôle</label>
                <input type="date" name="date_controle" class="w-full px-4 py-2.5 border border-gray-200 rounded-xl input-focus transition bg-white">
              </div>
              <div>
                <label class="block text-sm font-medium text-gray-700 mb-1">Service de contrôle</label>
                <input type="text" name="service_controle" class="w-full px-4 py-2.5 border border-gray-200 rounded-xl input-focus transition bg-white" placeholder="Cardiologie, etc.">
              </div>
            </div>

            <div class="flex flex-col sm:flex-row gap-3 pt-4">
              <button type="submit" class="flex-1 py-3 bg-indigo-600 text-white rounded-xl font-medium hover:bg-indigo-700 transition">
        ...
        <div class="bg-white rounded-2xl shadow-xl p-6 mt-6">
          <h3 class="font-semibold text-gray-800 mb-4">Comptes-rendus récents</h3>
          <div class="space-y-3">
            <div class="flex justify-between items-center p-3 bg-gray-50 rounded-lg">
              <div>
                <p class="text-sm font-medium text-gray-800">Patient : .....</p>
                <p class="text-xs text-gray-500">Généré le ..... - Hospitalisation du ..... au .....</p>
              </div>
              <div class="flex gap-2">
                <a href="#" class="text-indigo-600 text-xs hover:underline">Voir</a>
                <a href="#" class="text-gray-500 text-xs hover:underline">PDF</a>
              </div>
            </div>
          </div>
        </div>
