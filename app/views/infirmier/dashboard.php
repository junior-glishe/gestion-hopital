...
<?php $titre = "Dashboard Infirmier - MediTrace";
ob_start(); ?>
<style>
  body {
...
<div class="flex">
  ...
  <aside class="w-72 bg-white shadow-lg min-h-screen sticky top-0">
    <div class="p-6 border-b border-gray-100">
      ...
      <div class="mt-4 p-3 bg-gray-50 rounded-xl">
        ...
        <p class="text-sm font-medium text-gray-700"><?= htmlspecialchars($nomComplet ?? 'Infirmier') ?></p>
    ...
    <nav class="p-4">
      ...
      <a href="index.php?p=infirmier/dashboard" class="sidebar-item active flex items-center gap-3 px-3 py-2.5 rounded-lg text-gray-700 mb-1">
  ...
  <main class="flex-1">
    <div class="max-w-7xl mx-auto px-4 py-8">
      ...
      <!-- Section Bienvenue -->
      <div class="bg-gradient-to-r from-teal-700 to-teal-600 rounded-2xl p-6 md:p-8 text-white mb-8 shadow-xl">
        <div class="flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
          <div>
            <h1 class="text-2xl md:text-3xl font-bold">Bonjour, <?= htmlspecialchars($nomComplet ?? 'Infirmier') ?></h1>
            <p class="text-teal-100 mt-1">Bienvenue sur votre espace infirmier MediTrace</p>
            <p class="text-teal-100/70 text-sm mt-2">Service : <span class="font-medium"><?= htmlspecialchars($service ?? 'Soins généraux') ?></span></p>
          </div>
          <div class="bg-white/10 backdrop-blur-sm rounded-xl px-4 py-2 text-center">
            <p class="text-xs text-teal-100">Dernière connexion</p>
        ...
        <div class="bg-white rounded-xl p-4 shadow-sm">
          <div class="flex items-center gap-3">
            ...
            <div>
              ...
              <p class="text-xs text-gray-500">Patients pris en charge</p>
            </div>
          </div>
        </div>
        <div class="bg-white rounded-xl p-4 shadow-sm">
          <div class="flex items-center gap-3">
            ...
            <div>
              <p class="text-2xl font-bold text-gray-800"><?= $nbConstantes ?? 0 ?></p>
              <p class="text-xs text-gray-500">Constantes enregistrées</p>
            </div>
          </div>
        </div>
        <div class="bg-white rounded-xl p-4 shadow-sm">
          <div class="flex items-center gap-3">
            ...
            <div>
              <p class="text-2xl font-bold text-gray-800"><?= $nbSoins ?? 0 ?></p>
              <p class="text-xs text-gray-500">Soins administrés</p>
            </div>
          </div>
        </div>
        <div class="bg-white rounded-xl p-4 shadow-sm">
          <div class="flex items-center gap-3">
            ...
            <div>
              ...
              <p class="text-xs text-gray-500">Alertes/Urgences</p>
            </div>
          </div>
        </div>
      </div>
      ...
      <div class="bg-white rounded-xl shadow-sm p-5 mb-8">
        <div class="flex justify-between items-center mb-4">
          <h3 class="font-semibold text-gray-800">Patients hospitalisés</h3>
          <a href="#" class="text-xs text-teal-600 hover:underline">Voir tous</a>
        </div>
            ...
            <tbody class="divide-y divide-gray-100">
              <?php if (!empty($patients)): ?>
                <?php foreach (array_slice($patients, 0, 5) as $patient): ?>
                  <tr>
                    <td class="px-4 py-3 text-sm text-gray-700"><?= htmlspecialchars(($patient['prenom'] ?? '') . ' ' . ($patient['nom'] ?? '')) ?><br><span class="text-xs text-gray-400">N° patient: <?= $patient['id'] ?? '-' ?></span></td>
                    <td class="px-4 py-3 text-sm text-gray-700">-</td>
                    <td class="px-4 py-3 text-sm text-gray-700"><?= htmlspecialchars($patient['date_naissance'] ?? '-') ?></td>
                    <td class="px-4 py-3 text-sm text-gray-700">-</td>
                    <td class="px-4 py-3">
                      <div class="flex gap-2">
                        <a href="index.php?p=infirmier/constantes_vitales" class="text-teal-600 text-xs hover:underline">Constantes</a>
                        <a href="index.php?p=infirmier/soins_surveillance" class="text-teal-600 text-xs hover:underline">Soins</a>
                      </div>
                    </td>
                  </tr>
                <?php endforeach; ?>
              <?php else: ?>
                <tr>
                  <td class="px-4 py-3 text-sm text-gray-500" colspan="5">Aucun patient enregistré</td>
                </tr>
              <?php endif; ?>
            </tbody>
          </table>
      ...
      <div class="bg-white rounded-xl shadow-sm p-5">
        <div class="flex justify-between items-center mb-4">
          <h3 class="font-semibold text-gray-800">Alertes récentes</h3>
          <a href="index.php?p=infirmier/signalement_urgence" class="text-xs text-teal-600 hover:underline">Voir tout</a>
        </div>
        <div class="space-y-3">
          <?php if (!empty($dernieresUrgences)): ?>
            <?php foreach ($dernieresUrgences as $urgence): ?>
              <div class="flex items-center justify-between p-3 bg-red-50 rounded-lg border border-red-100">
                <div>
                  <p class="text-sm font-medium text-red-800">Patient : <?= htmlspecialchars(($urgence['patient_prenom'] ?? '') . ' ' . ($urgence['patient_nom'] ?? '')) ?></p>
                  <p class="text-xs text-red-600"><?= htmlspecialchars($urgence['description'] ?? 'Urgence signalée') ?></p>
                </div>
                <span class="text-xs text-red-500"><?= htmlspecialchars(date('d/m/Y H:i', strtotime($urgence['cree_le'] ?? 'now'))) ?></span>
              </div>
            <?php endforeach; ?>
          <?php else: ?>
            <div class="flex items-center justify-between p-3 bg-gray-50 rounded-lg border border-gray-100">
              <div>
                <p class="text-sm font-medium text-gray-800">Aucune alerte récente</p>
