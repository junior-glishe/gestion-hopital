...
<?php $titre = "Demander sortie - MediTrace"; ob_start(); ?>
<style>
    body {
  ...
  <div class="flex">
    ...
    <aside class="w-72 bg-white shadow-lg min-h-screen sticky top-0">
      ...
      <nav class="p-4">
        ...
        <a href="index.php?p=patient/demander_sortie" class="sidebar-item active flex items-center gap-3 px-3 py-2.5 rounded-lg text-gray-700 mb-1">
          <svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.8">
    ...
    <main class="flex-1">
      <div class="max-w-3xl mx-auto px-4 py-8">
        ...
        <div class="flex items-center gap-2 text-sm text-gray-500 mb-6">
          ...
          <span class="text-gray-800 font-medium">Demander sortie</span>
        </div>
        ...
        <div class="bg-white rounded-2xl shadow-xl p-6 md:p-8">
          <div class="flex items-center gap-3 mb-6">
            ...
            <h1 class="text-xl md:text-2xl font-bold text-gray-800">Demander ma sortie</h1>
          </div>
          ...
          <div class="bg-blue-50 rounded-xl p-4 mb-6 border border-blue-100">
            <div class="flex items-start gap-3">
              ...
              <div>
                <p class="text-sm font-semibold text-blue-800">Hospitalisation en cours</p>
                <p class="text-xs text-gray-600 mt-1">Date d'admission : <span class="font-medium">.....</span></p>
                <p class="text-xs text-gray-600">Chambre : <span class="font-medium">.....</span></p>
                <p class="text-xs text-gray-600">Médecin responsable : <span class="font-medium">.....</span></p>
              </div>
            </div>
          </div>
...
<?php /* CSRF */ ?><input type="hidden" name="_csrf" value="<?= htmlspecialchars($csrf_token ?? '', ENT_QUOTES) ?>">
            ...
            <div>
              <label class="block text-sm font-medium text-gray-700 mb-1">Date de sortie souhaitée *</label>
              <input type="date" name="date_sortie" required class="w-full px-4 py-2.5 border border-gray-200 rounded-xl input-focus transition bg-white">
              <p class="text-xs text-gray-400 mt-1">La sortie ne pourra être effective qu'après validation médicale</p>
            </div>

            <!-- Motif de la sortie -->
            <div>
              <label class="block text-sm font-medium text-gray-700 mb-1">Motif de la demande de sortie *</label>
              <textarea name="motif" rows="4" required class="w-full px-4 py-2.5 border border-gray-200 rounded-xl input-focus transition bg-white" placeholder="Ex: Fin de traitement, transfert vers un autre établissement, retour à domicile..."></textarea>
            </div>

            <!-- Adresse de sortie -->
            <div>
              <label class="block text-sm font-medium text-gray-700 mb-1">Adresse de sortie (domicile)</label>
              <input type="text" name="adresse_sortie" class="w-full px-4 py-2.5 border border-gray-200 rounded-xl input-focus transition bg-white" placeholder="Votre adresse complète après la sortie">
            </div>
            ...
            <div class="grid md:grid-cols-2 gap-4">
              <div>
                ...
                <input type="text" name="contact_nom" class="w-full px-4 py-2.5 border border-gray-200 rounded-xl input-focus transition bg-white" placeholder="Nom et prénom">
              </div>
              <div>
                <label class="block text-sm font-medium text-gray-700 mb-1">Téléphone du contact</label>
                <input type="tel" name="contact_tel" class="w-full px-4 py-2.5 border border-gray-200 rounded-xl input-focus transition bg-white" placeholder="Téléphone">
              </div>
            </div>

            <!-- Moyen de transport -->
            <div>
              <label class="block text-sm font-medium text-gray-700 mb-1">Moyen de transport pour la sortie</label>
              <select name="transport" class="w-full px-4 py-2.5 border border-gray-200 rounded-xl input-focus transition bg-white">
                <option value="">Sélectionnez</option>
                <option value="famille">Prise en charge par la famille</option>
            ...
            <div class="space-y-3">
              <label class="flex items-center gap-3">
                ...
                <span class="text-sm text-gray-600">Je confirme avoir réglé l'intégralité de ma facture d'hospitalisation</span>
              </label>
              <label class="flex items-center gap-3">
                <input type="checkbox" required class="w-4 h-4 rounded border-gray-300 text-green-700">
                <span class="text-sm text-gray-600">Je m'engage à respecter les recommandations médicales post-sortie</span>
              </label>
            </div>

            <!-- Boutons -->
            <div class="flex flex-col sm:flex-row gap-3 pt-4">
              <button type="submit" class="flex-1 py-3 bg-orange-600 text-white rounded-xl font-medium hover:bg-orange-700 transition">
                Envoyer la demande
              </button>
          ...
          <div class="mt-6 p-4 bg-orange-50 rounded-xl border border-orange-100">
            <div class="flex items-start gap-3">
              <svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="#ea580c" stroke-width="1.8">
                <circle cx="12" cy="12" r="10"/>
                <path d="M12 8v4l3 3"/>
              </svg>
              <div>
                <p class="text-sm font-medium text-orange-800">Informations importantes</p>
                <p class="text-xs text-orange-700 mt-1">La sortie doit être validée par votre médecin traitant.</p>
                <p class="text-xs text-orange-700 mt-1">Un compte-rendu d'hospitalisation vous sera remis avant votre départ.</p>
                <p class="text-xs text-orange-700 mt-1">Veuillez vous assurer que toutes vos affaires personnelles sont rassemblées.</p>
              </div>
            </div>
          </div>
        </div>
