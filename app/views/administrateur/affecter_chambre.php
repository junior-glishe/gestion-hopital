...
<?php $titre = "Affecter chambre - MediTrace"; ob_start(); ?>
<style>
...
<aside class="w-72 bg-white shadow-lg min-h-screen sticky top-0">
  ...
  <nav class="p-4">
    ...
    <a href="index.php?p=administrateur/affecter_chambre" class="sidebar-item active flex items-center gap-3 px-3 py-2.5 rounded-lg text-gray-700 mb-1"><svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.8"><rect x="3" y="3" width="18" height="18" rx="2"/><path d="M3 9H21"/><path d="M9 21V9"/></svg><span>Affecter chambre</span></a>
    ...
    <a href="index.php?p=administrateur/planifier_rendezvous" class="sidebar-item flex items-center gap-3 px-3 py-2.5 rounded-lg text-gray-700 mb-1"><svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.8"><circle cx="12" cy="12" r="10"/><path d="M12 8v4l3 3"/></svg><span>Planifier rendez-vous</span></a>
...
<main class="flex-1">
  <div class="max-w-4xl mx-auto px-4 py-8">
    <div class="flex items-center gap-2 text-sm text-gray-500 mb-6">
      ...
      <span class="text-gray-800 font-medium">Affecter chambre</span>
    </div>

    <div class="bg-white rounded-2xl shadow-xl p-6 md:p-8">
      <div class="flex items-center gap-3 mb-6"><div class="w-10 h-10 bg-purple-100 rounded-xl flex items-center justify-center"><svg width="22" height="22" viewBox="0 0 24 24" fill="none" stroke="#7c3aed" stroke-width="1.8"><rect x="3" y="3" width="18" height="18" rx="2"/><path d="M3 9H21"/><path d="M9 21V9"/></svg></div><h1 class="text-xl md:text-2xl font-bold text-gray-800">Affecter une chambre</h1></div>

      <form action="" method="POST" class="space-y-5">
<?php /* CSRF */ ?><input type="hidden" name="_csrf" value="<?= htmlspecialchars($csrf_token ?? '', ENT_QUOTES) ?>">
        <div><label class="block text-sm font-medium text-gray-700 mb-1">Patient *</label><select name="patient" required class="w-full px-4 py-2.5 border border-gray-200 rounded-xl input-focus transition"><option value="">Sélectionnez un patient</option><option value=".....">..... - ..... - .....</option><option value=".....">..... - ..... - .....</option></select></div>
        <div class="grid md:grid-cols-2 gap-4">
          <div><label class="block text-sm font-medium text-gray-700 mb-1">Chambre *</label><select name="chambre" required class="w-full px-4 py-2.5 border border-gray-200 rounded-xl input-focus transition"><option value="">Sélectionnez une chambre</option><option value="101">101 - Standard (25 000 FCFA/jour)</option><option value="102">102 - Standard (25 000 FCFA/jour)</option><option value="201">201 - Double (40 000 FCFA/jour)</option><option value="301">301 - VIP (75 000 FCFA/jour)</option></select></div>
          <div><label class="block text-sm font-medium text-gray-700 mb-1">Date d'admission *</label><input type="date" name="date_admission" required class="w-full px-4 py-2.5 border border-gray-200 rounded-xl input-focus transition"></div>
        </div>
        <div><label class="block text-sm font-medium text-gray-700 mb-1">Médecin responsable *</label><select name="medecin" required class="w-full px-4 py-2.5 border border-gray-200 rounded-xl input-focus transition"><option value="">Sélectionnez un médecin</option><option value=".....">Dr ..... - Cardiologue</option><option value=".....">Dr ..... - Généraliste</option></select></div>
        <div><label class="block text-sm font-medium text-gray-700 mb-1">Diagnostic / Motif</label><textarea name="diagnostic" rows="3" class="w-full px-4 py-2.5 border border-gray-200 rounded-xl input-focus transition" placeholder="Diagnostic ou motif d'hospitalisation..."></textarea></div>
        <button type="submit" class="w-full py-3 bg-green-700 text-white rounded-xl font-medium hover:bg-green-800 transition">Affecter la chambre</button>
      </form>

      <div class="mt-6 p-4 bg-blue-50 rounded-xl"><div class="flex items-start gap-3"><svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="#2563eb" stroke-width="1.8"><circle cx="12" cy="12" r="10"/><path d="M12 8v4l3 3"/></svg><div><p class="text-sm font-medium text-blue-800">Liste des chambres disponibles</p><div class="grid grid-cols-3 gap-2 mt-2"><span class="text-xs bg-white px-2 py-1 rounded text-gray-600">101 - Standard</span><span class="text-xs bg-white px-2 py-1 rounded text-gray-600">102 - Standard</span><span class="text-xs bg-white px-2 py-1 rounded text-gray-600">201 - Double</span></div></div></div></div>
    </div>
  </div>
