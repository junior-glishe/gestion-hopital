...
<?php $titre = "Générer facture - MediTrace"; ob_start(); ?>
<style>
...
<aside class="w-72 bg-white shadow-lg min-h-screen sticky top-0">
  ...
  <nav class="p-4">
    ...
    <a href="index.php?p=administrateur/generer_facture" class="sidebar-item active flex items-center gap-3 px-3 py-2.5 rounded-lg text-gray-700 mb-1"><svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.8"><rect x="4" y="4" width="16" height="20" rx="2"/><path d="M8 12H16"/><path d="M12 8V16"/></svg><span>Générer facture</span></a>
...
<main class="flex-1">
  <div class="max-w-3xl mx-auto px-4 py-8">
    <div class="flex items-center gap-2 text-sm text-gray-500 mb-6"><a href="index.php?p=administrateur/dashboard" class="hover:text-green-700">Tableau de bord</a><svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path d="M9 18l6-6-6-6"/></svg><span class="text-gray-800 font-medium">Générer facture</span></div>
    <div class="bg-white rounded-2xl shadow-xl p-6 md:p-8">
      <div class="flex items-center gap-3 mb-6"><div class="w-10 h-10 bg-yellow-100 rounded-xl flex items-center justify-center"><svg width="22" height="22" viewBox="0 0 24 24" fill="none" stroke="#ca8a04" stroke-width="1.8"><rect x="4" y="4" width="16" height="20" rx="2"/><path d="M8 12H16"/><path d="M12 8V16"/></svg></div><h1 class="text-xl md:text-2xl font-bold text-gray-800">Générer une facture</h1></div>
      <form action="" method="POST" class="space-y-5">
<?php /* CSRF */ ?><input type="hidden" name="_csrf" value="<?= htmlspecialchars($csrf_token ?? '', ENT_QUOTES) ?>">
        <div><label class="block text-sm font-medium text-gray-700 mb-1">Patient *</label><select name="patient" required class="w-full px-4 py-2.5 border border-gray-200 rounded-xl input-focus transition"><option value="">Sélectionnez un patient</option><option value=".....">..... - ..... </option></select></div>
        <div><label class="block text-sm font-medium text-gray-700 mb-1">Type de prestation *</label><select name="type_prestation" required class="w-full px-4 py-2.5 border border-gray-200 rounded-xl input-focus transition"><option value="">Sélectionnez</option><option value="consultation">Consultation</option><option value="hospitalisation">Hospitalisation</option><option value="examen">Examens médicaux</option><option value="pharmacie">Pharmacie</option></select></div>
        <div class="grid md:grid-cols-2 gap-4"><div><label class="block text-sm font-medium text-gray-700 mb-1">Montant *</label><input type="number" name="montant" required class="w-full px-4 py-2.5 border border-gray-200 rounded-xl input-focus transition"></div><div><label class="block text-sm font-medium text-gray-700 mb-1">Assurance</label><select name="assurance" class="w-full px-4 py-2.5 border border-gray-200 rounded-xl input-focus transition"><option value="">Aucune</option><option value="CNSS">CNSS (70%)</option><option value="RAMED">RAMED (100%)</option><option value="Mutuelle">Mutuelle (80%)</option></select></div></div>
        <div><label class="block text-sm font-medium text-gray-700 mb-1">Description</label><textarea name="description" rows="3" class="w-full px-4 py-2.5 border border-gray-200 rounded-xl input-focus transition" placeholder="Détails de la prestation..."></textarea></div>
        <div class="bg-gray-50 p-4 rounded-xl"><div class="flex justify-between text-sm"><span class="text-gray-600">Sous-total</span><span class="font-medium">..... FCFA</span></div><div class="flex justify-between text-sm mt-2"><span class="text-gray-600">Prise en charge assurance</span><span class="text-green-700">..... FCFA</span></div><div class="flex justify-between font-bold mt-3 pt-2 border-t"><span>Total à payer</span><span class="text-red-700">..... FCFA</span></div></div>
        <button type="submit" class="w-full py-3 bg-green-700 text-white rounded-xl font-medium hover:bg-green-800 transition">Générer la facture</button>
      </form>
      <div class="mt-4 text-center"><a href="#" class="text-sm text-green-700 hover:underline">📄 Voir les factures existantes</a></div>
    </div>
  </div>
