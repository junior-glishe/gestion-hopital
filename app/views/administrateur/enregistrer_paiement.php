...
<?php $titre = "Enregistrer paiement - MediTrace"; ob_start(); ?>
<style>
...
<aside class="w-72 bg-white shadow-lg min-h-screen sticky top-0">
  ...
  <nav class="p-4">
    ...
    <a href="index.php?p=administrateur/enregistrer_paiement" class="sidebar-item active flex items-center gap-3 px-3 py-2.5 rounded-lg text-gray-700 mb-1"><svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.8"><circle cx="12" cy="12" r="3"/><path d="M3 12H7M17 12H21"/></svg><span>Enregistrer paiement</span></a>
...
<main class="flex-1">
  <div class="max-w-3xl mx-auto px-4 py-8">
    <div class="flex items-center gap-2 text-sm text-gray-500 mb-6"><a href="index.php?p=administrateur/dashboard" class="hover:text-green-700">Tableau de bord</a><svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path d="M9 18l6-6-6-6"/></svg><span class="text-gray-800 font-medium">Enregistrer paiement</span></div>
    <div class="bg-white rounded-2xl shadow-xl p-6 md:p-8">
      <div class="flex items-center gap-3 mb-6"><div class="w-10 h-10 bg-green-100 rounded-xl flex items-center justify-center"><svg width="22" height="22" viewBox="0 0 24 24" fill="none" stroke="#0d6b4e" stroke-width="1.8"><circle cx="12" cy="12" r="3"/><path d="M3 12H7M17 12H21"/></svg></div><h1 class="text-xl md:text-2xl font-bold text-gray-800">Enregistrer un paiement</h1></div>
      <form action="" method="POST" class="space-y-5">
<?php /* CSRF */ ?><input type="hidden" name="_csrf" value="<?= htmlspecialchars($csrf_token ?? '', ENT_QUOTES) ?>">
        <div><label class="block text-sm font-medium text-gray-700 mb-1">Facture *</label><select name="facture" required class="w-full px-4 py-2.5 border border-gray-200 rounded-xl input-focus transition"><option value="">Sélectionnez une facture</option><option value=".....">FACT-001 - ..... - ..... FCFA</option><option value=".....">FACT-002 - ..... - ..... FCFA</option></select></div>
        <div class="grid md:grid-cols-2 gap-4">
          <div><label class="block text-sm font-medium text-gray-700 mb-1">Montant payé *</label><input type="number" name="montant" required class="w-full px-4 py-2.5 border border-gray-200 rounded-xl input-focus transition"></div>
          <div><label class="block text-sm font-medium text-gray-700 mb-1">Date du paiement *</label><input type="date" name="date_paiement" required class="w-full px-4 py-2.5 border border-gray-200 rounded-xl input-focus transition"></div>
        </div>
        <div class="grid md:grid-cols-2 gap-4">
          <div><label class="block text-sm font-medium text-gray-700 mb-1">Mode de paiement *</label><select name="mode_paiement" required class="w-full px-4 py-2.5 border border-gray-200 rounded-xl input-focus transition"><option value="">Sélectionnez</option><option value="especes">Espèces</option><option value="carte">Carte bancaire</option><option value="mobile_money">Mobile Money</option><option value="virement">Virement bancaire</option></select></div>
          <div><label class="block text-sm font-medium text-gray-700 mb-1">Référence transaction</label><input type="text" name="reference" class="w-full px-4 py-2.5 border border-gray-200 rounded-xl input-focus transition" placeholder="Optionnel"></div>
        </div>
        <div class="bg-gray-50 p-4 rounded-xl"><div class="flex justify-between text-sm"><span class="text-gray-600">Montant total de la facture</span><span class="font-medium">..... FCFA</span></div><div class="flex justify-between text-sm mt-2"><span class="text-gray-600">Déjà payé</span><span class="font-medium">..... FCFA</span></div><div class="flex justify-between text-sm mt-2"><span class="text-gray-600">Reste à payer</span><span class="text-red-700">..... FCFA</span></div></div>
        <button type="submit" class="w-full py-3 bg-green-700 text-white rounded-xl font-medium hover:bg-green-800 transition">Enregistrer le paiement</button>
      </form>
    </div>
  </div>
